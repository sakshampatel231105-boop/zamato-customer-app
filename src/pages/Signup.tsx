...
function Signup({ onAuth }: SignupProps) {
  ...
  const loginHandler = async (e: React.FormEvent) => {
   ...
   try {
     // URL ko correct karke sahi endpoint '/api/vendors/signup' kar diya hai
     const response = await fetch('https://zamato-backend.onrender.com/api/vendors/signup', {
          method: 'POST',
          body: formData,
     });
        ...
        alert('Restaurant Partner signed up successfully');

        onAuth(ans.vendor || ans.user, ans.token);
        setMobile('');
        setPassword('');
