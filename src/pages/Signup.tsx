...
function Signup({ onAuth }: SignupProps) {
  ...
  const loginHandler = async (e: React.FormEvent) => {
    ...
    try {
      // Backend userRoutes ke mutabik bilkul sahi route: /api/users/signup
      const response = await fetch('https://zamato-backend.onrender.com/api/users/signup', {
        method: 'POST',
        body: formData,
      });
      ...
      <div className="flex items-center justify-center h-screen">
        <form className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" onSubmit={loginHandler}>
          ...
          <p className="mt-4 text-sm text-gray-600">
            ...
            <Link to="/login" className="text-black hover:text-blue-700 cursor-pointer">Login</Link>
          </p>
        </form>
      </div>
    </div>
